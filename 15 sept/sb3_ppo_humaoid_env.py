...
env=HumanoidEnv()
vec_env = make_vec_env(ENV, n_envs=6)
model = PPO("MlpPolicy", vec_env, verbose=1)
model.learn(total_timesteps=total_timesteps)
model.save(f"humanoid_sbPPO robot_standing.xml done= {env.timestep} vecenv 4 envs {total_timesteps}")
# del model
model = PPO.load(f"humanoid_sbPPO robot_standing.xml done= {env.timestep} vecenv 4 envs {total_timesteps}")
#
obs,_ = env.reset()
env.render("human")
while True:
